...
for f in docs/開發計畫書.md docs/任務表.md docs/TDD測試計畫表.md docs/SSO接入申請.md docs/plans/帳號落地統一與組織部門對齊計畫書.md README.md; do
  if [ -f "$f" ]; then check_meta "$f"; else ng "$f" "檔案不存在"; fi
done

echo "[3] 正式文件 HTML 發布版存在(第四條4)"
for f in docs/開發計畫書.html docs/任務表.html docs/TDD測試計畫表.html docs/SSO接入申請.html docs/plans/帳號落地統一與組織部門對齊計畫書.html README.html; do
