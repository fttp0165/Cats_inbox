...
| T10c | 🔴 `test_release.py::test_compose_image_tag_matches_version_constant` | compose 的映像 tag == `__version__` | 🔴 **最安靜的那種錯**:改了常數而忘了 compose → 部署的是**舊映像**,而它的 health 回報**舊版本**,兩者「看起來完全一致」,而你以為換版成功了 |
| T10c | 🔴 `test_release.py::test_never_pushes_latest` | workflow 全檔不得含 `latest=true` / `:latest` / `value=latest` | 平台紅線。`latest` 讓同一個 tag 在不同時間指向不同東西,**回滾時你不知道要回到哪裡** |
| T10c | `test_release.py::test_triggered_by_tag_push_only` | 只在推 tag 時建,且留 `workflow_dispatch` | Docker 建置會吃 Actions 額度(portal 因此紅了一週),而症狀是「發版當下才發現建不出來」;而 tag 不能重發,所以要有補建入口 |
| T10c | `test_release.py::test_workflow_verifies_tag_matches_version_constant` | workflow 讀 `__version__` 且不一致時 `exit 1` | 第八條4。**只 echo 一行不算** —— 那不會讓建置失敗 |
| T10c | 🔴 `test_release.py::test_smoke_step_runs_the_image_before_pushing` | 有 `docker run` + 打 `/inbox/health`,且 `load: true` 在 `push: true` **之前** | 「建得起來」與「跑得起來」是兩件事。壞掉的映像被推上去之後,發現的時機是**低峰窗口裡** |
| T10c | `test_release.py::test_every_run_block_is_valid_shell` | 每個 `run:` 區塊過 `bash -n` | 語法錯**只在發版當下**顯現,而那時 tag 已經推了。⚠ `bash -n` 只驗語法不驗語意(缺 `]` 它認為合法);兩個 `sed` 另外在本機對真檔案實測 |
| T10c | 🔴 `test_release.py::test_release_sop_exists_with_four_required_sections` | SOP 有第八條的 **`## 標題`** 四段 + title 格式 + **誠實標註哪些沒跑過** | ⚠ 第一版驗「字串在不在全文裡」,而「部署注意」在別處說明文字裡也有 —— 改標題名後仍然綠。**斷言的粒度比它宣稱保護的性質粗**(T09b/T10b 同型,連續第三輪) |
| T10c | `test_docs.sh`(**改寫**) | 文件鏈清單**自 `render_docs.py` 的 `TARGETS` 產生**,並斷言解析到 ≥7 份 | 🔴 D01 記過:新文件不登記進清單,`--check` 會全綠。而清單原本手寫三次(新增文件要改四個地方)。⚠ 解析失敗會讓清單變空,而**空清單的迴圈是全綠的** |
...
### 5.12 突變檢查(T10c,2026-08-25)

**15 項全被抓到**(修掉一個測試洞、修正兩處工具錯誤之後)。三項值得記:

| 故意改壞 | 對應測試 | 結果 |
|---|---|---|
| 改了 `__version__` 而忘了 compose | `test_compose_image_tag_matches_version_constant` | ✅ 紅 |
| 拿掉推之前的冒煙 | `test_smoke_step_runs_the_image_before_pushing` | ✅ 紅 |
| 文件清單解析失敗(守門變成空檢查) | `test_docs.sh` 的數量下限斷言 | ✅ 紅 |

🔴 **一項測試洞:SOP 的四段檢查驗「字串在不在全文裡」,不是「有沒有那個標題」。**
把 `## 部署注意` 改成 `## 其他` 之後那一條仍然綠 —— 因為那四個字在別處的
說明文字裡也出現過。已改成驗 `## 部署注意`。

⚠ **這是連續第三輪出現同一種形狀**(§5.10 三個、§5.11 一個、本輪一個):
**斷言的粒度比它宣稱保護的性質粗。**
「字串在不在全文」比「那個標題存在」粗;「欄位名在不在」比「token 有效」粗;
「整頁有沒有那個詞」比「錯誤框裡寫了什麼」粗。
🔴 **寫斷言時要問:我要保護的性質,能不能被一個更粗的東西湊巧滿足?**

⚠ 另有兩處**工具錯誤**,而守門的警示行都正確地講出來了:
①我以為「缺 `]`」是語法錯,而 `bash -n` 認為合法(`[` 是命令不是語法);
...
| 版本 | 日期 | 修改人 | 摘要 |
|---|---|---|---|
| v1.13 | 2026-09-23 | Benny | **T14a 完工回寫**:§2 對應表改寫推送列;§3 原 6 列 T14 → **T14a 26 列**(6 支核定規格保留原斷言並具體化 + 16 支自加 + 4 支 PG schema),逐支寫出「壞掉時沒有症狀」的理由;§5 覆蓋現況加**推送層 22 項**、schema 層 10 → 14;🔴 **更正一句自 T05b 起就不成立的話**:migration 層那列仍寫「PG15 未演練——留 T11 於 VM 補」,而 CI 自 `ci #46` 起每次 push 都跑 PG 15.8(T05b 回寫了四份文件,**本表不在那一批裡**);新增 §5.13 突變檢查 **36/36**,記兩個形狀:「兩道守門擋同一個案例」(單獨拿掉任一道都不會紅)與「抓到了要看是哪一行抓到的」 ⚠ **時間戳更正**(非實質變更,不升版):「最後更新」原寫 `15:25` —— 寫的是**預估**的完工時刻,而本版實際於 commit `6e5176b`(15:21)定稿;已改為以開發機 `date` 為準(契約 v2.16 記過的同一種錯)。 |
